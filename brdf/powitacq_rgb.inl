...
std::vector<float> BRDF::eval(const std::vector<float> &vwi, const std::vector<float> &vwo) const {
    Vector3f wi = Vector3f(vwi.at(0), vwi.at(1), vwi.at(2));
    Vector3f wo = Vector3f(vwo.at(0), vwo.at(1), vwo.at(2));
    if (wi.z() < 0 || wo.z() < 0) {
        Vector3f z = zero();
        std::vector<float> vz = {z[0], z[1], z[2]};
        return vz;
    }
    ...
    std::vector<float> vfr = {fr[0], fr[1], fr[2]};
